...
def convert_face_on_screen_to_angle_in_x(face, screen_width, angle_screen, logger):
    ...
    diffAngleX = xDiff * PI / (IMAGE_WIDTH * 8)
...
def convert_face_on_screen_to_angle_in_y(face, screen_height, angle_screen, logger):
    ...
    diffAngleY = yDiff * PI / (IMAGE_HEIGHT * 8)
            ...
            if self.data == '':
                ...
                if(self.face_queue.empty() == False):
                    ...
                    if isinstance(face, processing.face.Face):
                        currentRadianValueX = currentRadianValueX + convert_face_on_screen_to_angle_in_x(face, IMAGE_WIDTH, IMAGE_TOTAL_ANGLE, info_logger)
                        currentRadianValueY = currentRadianValueY - convert_face_on_screen_to_angle_in_y(face, IMAGE_HEIGHT, IMAGE_TOTAL_ANGLE, info_logger)
                    else:
                    ...
                    if((abs(currentRadianValueX - lastSentValueX) and abs(currentRadianValueY - lastSentValueY)) > MINIMUM_ANGLE_MOVEMENT):
